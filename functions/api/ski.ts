...
export const onRequest: PagesFunction<{
  ...
  const resorts = RESORTS.filter((r) => (only.length ? only.includes(r.id) : true));
  ...
  return new Response(JSON.stringify({
    ...
    ttl_seconds: Number(ctx.env.SKI_CACHE_TTL_SECONDS || "10800"),
    results,
  }), {
    headers: {
...
const RESORTS: Resort[] = [
  ...
  { id: "hunter", name: "Hunter Mountain", url: "https://www.huntermtn.com/mountain-report/", family: "vail" },
  { id: "stowe", name: "Stowe", url: "https://www.stowe.com/the-mountain/mountain-report/", family: "vail" },
  { id: "okemo", name: "Okemo", url: "https://www.okemo.com/the-mountain/mountain-report/", family: "vail" },
  { id: "mountsnow", name: "Mount Snow", url: "https://www.mountsnow.com/the-mountain/mountain-report/", family: "vail" },
  { id: "wildcat", name: "Wildcat Mountain", url: "https://www.skiwildcat.com/the-mountain/mountain-report/", family: "vail" },
  { id: "stratton", name: "Stratton", url: "https://www.stratton.com/the-mountain/mountain-report", family: "vail" },

  // Alterra Resorts (Ikon Pass)
  { id: "killington", name: "Killington", url: "https://www.killington.com/the-mountain/mountain-conditions/snow-and-weather-report", family: "alterra" },

  // Camelback - Custom scraper (proven working)
  { id: "camelback", name: "Camelback", url: "https://conditions.camelbackresort.com/conditions/snow-report/printable-report/", family: "camelback_print" },
];
...
async function getResortSnapshot(
  ...
  snow24_in = matchInchesNear(text, /(24\s*hr|24-hour|past\s*24\s*hours|overnight)/i);
  ...
  await ctx.env.SKI_KV.put(key, JSON.stringify(snapshot), { expirationTtl: ttl });
  return snapshot;
}
...
async function fetchHtml(url: string) {
  const resp = await fetch(url, {
    headers: {
      "user-agent": "Mozilla/5.0 (compatible; IceCoastBot/1.0; +https://icecoast.ski)",
...
function computeRating(sig: {
  ...
  if (openPct > 0 && openPct < 0.25) return { rating: "limited", confidence: 0.75 };
  ...
  if ((s24 != null && s24 >= 3) || (s48 != null && s48 >= 5) || (s7 != null && s7 >= 10)) {
...
function mapSurface(surface: string): string | null {
  ...
  if (surface.includes("machine groomed") || surface.includes("groomed")) return "machine_groomed";
  if (surface.includes("hard pack") || surface.includes("hardpack") || surface.includes("firm")) return "firm_hardpack";
...
function extractSurfaceText(text: string): string | undefined {
  ...
  for (const re of candidates) {
    ...
    if (idx >= 0) {
      ...
      const m = window.match(/(powder|packed powder|machine groomed|groomed|hard ?pack|firm|spring|icy)[^\.]{0,40}/i);
...
function matchFractionNear(text: string, label: RegExp): { open: number; total: number } | null {
  // Pattern 1: "12 / 15 lifts open"
  const m1 = text.match(new RegExp(`(\\d+)\\s*\\/\\s*(\\d+)\\s*${label.source}`, "i"));
  if (m1) return { open: Number(m1[1]), total: Number(m1[2]) };

  // Pattern 2: "lifts open 12 / 15"
  const m2 = text.match(new RegExp(`${label.source}\\s*(\\d+)\\s*\\/\\s*(\\d+)`, "i"));
  if (m2) return { open: Number(m2[1]), total: Number(m2[2]) };

  // Pattern 3: "11 of 12 lifts open"
  const m3 = text.match(new RegExp(`(\\d+)\\s*of\\s*(\\d+)\\s*${label.source}`, "i"));
  if (m3) return { open: Number(m3[1]), total: Number(m3[3]) };
